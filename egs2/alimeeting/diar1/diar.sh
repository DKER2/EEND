...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        log "Stage 1: Data preparation for data/${valid_set}, etc."
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        ...
        for dset in "${valid_set}" "${train_set}"; do
            ...
            #copy rttm file to ${data_feats}
            cp data/"${dset}"/rttm ${data_feats}${_suf}/${dset}/rttm
...
if ! "${skip_train}"; then
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        ...
        log "Stage 3: Diarization collect stats: train_set=${_diar_train_dir}, valid_set=${_diar_valid_dir}"
